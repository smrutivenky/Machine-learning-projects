...
def main(_):
 ...
 print("Logistic regression trained and tested on mnist")
...
#Logistic usps  
 ...
 image_labels = []
 i = 1
 for filename in glob.glob('../Test/*.png'): #assuming gif
     ...
     label = numpy.zeros((1,10))
     a = 10 - math.ceil(i/150)
     label[0,a] = 1
 ...
 correct_prediction = tf.equal(tf.argmax(y, 1), tf.argmax(y_, 1))
 accuracy = tf.reduce_mean(tf.cast(correct_prediction, tf.float32))
 print("Logistic regression tested on usps")
 print(sess.run(accuracy, feed_dict={x: image_list,
                                     y_: image_labels}))
 plt.plot(err)
 plt.show()
 ...
 with tf.Session() as sess:
      ...
      print("Single layer network on mnist data")
      print("Accuracy:", accuracy.eval({x: test_features, y: test_labels}))
 end = time.clock()
 print ("time:", end-start)
 #------------------------------------MLP on usps dataset-----------------------------------------------------------------
 test_features = image_list
 ...
 with tf.Session() as sess:
      ...
      correct_prediction = tf.equal(tf.argmax(pred, 1), tf.argmax(y, 1))
      accuracy = tf.reduce_mean(tf.cast(correct_prediction, "float"))
      print("Single layer network on usps data")
      print("Accuracy:", accuracy.eval({x: test_features, y: test_labels}))
 end = time.clock()
 print ("time:", end-start)
...
#-------------------------------------------------------------CNN on USPS-----------------------------------------------
 ...
 with tf.Session() as sess:
   ...
   print('test accuracy %g' % accuracy.eval(feed_dict={
       x: image_list, y_: image_labels, keep_prob: 1.0}))  
